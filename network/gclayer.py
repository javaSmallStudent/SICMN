...
def A_normalize(
    ...
    if self_loop_flag:
        matrix = matrix.cuda() + self_loop.cuda() # self.diagonal_i
    with torch.no_grad():
...
def gassian_general_A(heatmap, importance_batch):
    K = heatmap.shape[0]
    ...
    #
    threshold = 0.5  #
    importance = importance_batch[0][0]

    for i in range(K):
        for j in range(K):
            heatmap[i][j] = np.exp(-((i - j) ** 2) / (2 * sigma ** 2))


    #
    #A = (heatmap > threshold).astype(int)
    return heatmap


def instance_gassian(heatmap, pt, sigma):
    # Check that any part of the gaussian is in-bounds
    if math.isnan(pt[0] - 3 * sigma):
        print(f'pt: {pt}')
    ...
    # print(f'\nimg_x: {img_x}   img_y: {img_y}  heatmap.shape: {heatmap.shape} g_x:{g_x}  g_y:{g_y}\n')
    # print(f'g = {g.shape} \n')
    # print(f'pt = {pt}\n')
    # print(f'heartmap计算={heatmap[img_x[0]:img_x[1], img_y[0]:img_y[1]].shape}  g[g_x[0]:g_x[1]].view(-1, 1).shape={ g[g_x[0]:g_x[1]].view(-1, 1)}'
    #       f' g[g_y[0]:g_y[1]].view(1, -1)={g[g_y[0]:g_y[1]].view(1, -1).shape} ')
...
def build_adjacent_PriorImportance(
    ...
    for b in range(B):
        ...
        #adjacency_matrix_sample = gassian_general_A(adjacency_matrix_initial, importances_batch)
        ##
