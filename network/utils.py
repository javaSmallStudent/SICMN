...
def get_optim(model, args):
    if args.optim in ['Adam', 'adam']:
        ...
        optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, momentum=args.momen, weight_decay=args.wd)
    else:
...
def returnImportance(feature, weight_softmax, class_idx):
    ...
    channel_sum = torch.sum(importance_classes_tensor, dim=2, keepdim=False)
    importance_std = importance_classes_tensor / channel_sum

    # 
    uniform_importance = torch.ones((K))/float(K) #
    if importance_std.is_cuda:
        uniform_importance.cuda()
    importance_std = importance_std.squeeze(dim = 1)
    ...
    importance = torch.softmax(importance_std, dim=1)



    return importance_classes, importance    # [B, 3, K], [B, K]
...
def saveModel(model, epoch, model_save_path, loss_test_history_mean, c_index, type):

    """Save model."""
    if not os.path.exists('../model'):
        os.mkdir('../model')
    """Remove former models."""
    type_path = model_save_path + f'/{type}'
    if not os.path.exists(type_path):
        os.mkdir(type_path)
    for file_path in glob.glob(type_path + '/*.pth'):
        os.remove(file_path)

    torch.save(model.state_dict(), f'{type_path}/test_min_loss_epoch_{epoch + 1}_c_Index_{c_index:.3f}_loss_{loss_test_history_mean}.pth')

def save_log(recordtxt, tx, training=False):
    if training:
        ...
        with open(recordtxt, 'a') as f:
            title = ('%10s' * 6) % ('Test_C_index', 'Test_MAE', 'Test_MSE', 'Test_RMSE', 'Test_R2', 'Test_MAPE')
            f.write(title + '\n')
            f.write(tx + '\n')
            f.write('\n')
...
def load_model_pth(model, model_save_path):
    type_path = model_save_path
    for file_path in glob.glob(type_path + '/*.pth'):
